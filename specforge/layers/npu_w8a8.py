# coding=utf-8
"""Ascend NPU W8A8 inference layer for DFlash/Domino drafts.

``torch_npu`` is imported lazily so the module remains importable in CPU-only
...
class NPUQuantizedLinear(nn.Module):
    """NPU dynamic W8A8 replacement for ``nn.Linear``."""
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer("weight", weight_int8.to(device=device))
...
def replace_linear_with_npu_w8a8(
    module: nn.Module,
    w_bit: int = 8,
    ...
    """Replace every eligible ``nn.Linear`` with an NPU W8A8 layer."""
    import torch_npu  # noqa: F401  (validates NPU at call time)
    ...
    def walk(mod: nn.Module, path_prefix: str = "") -> None:
        for name, child in mod.named_children():
            ...
            if isinstance(child, nn.Linear):
                ...
                new = NPUQuantizedLinear(
                    child.in_features,
                    child.out_features,
                    bias=bias,
                    weight_int8=w_int8.t().contiguous(),
...
__all__ = ["NPUQuantizedLinear", "replace_linear_with_npu_w8a8"]
